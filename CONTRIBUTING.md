...
`bun x --bun commitlint` before any gate row, and `bun install` runs lefthook's postinstall, which
...
- A root `.env`. The gate refuses a tracked one, but your own untracked `.env` passes the
  ...
  `bun.lock` change, through `BUN_INSTALL_CACHE_DIR`, or `BUN_CONFIG_SKIP_LOAD_LOCKFILE` with a
  registry. It hands its values to every script it runs. `prepare` passes `--no-env-file`, so
  `bun run prepare` hands lefthook's `install` none of them, but under `bun install` lefthook still
  sees them.
- The Bun variables. `BUN_OPTIONS` reaches every direct Bun start, such as the `prepare` script's
  ...
  processes it starts, and a tool started through `bun x --bun --no-install` does not read it.
  Leave it unset. Bun preloads the module `BUN_INSPECT_PRELOAD` names, and reads `BUN_INSPECT` and
...
- No analyzer checks any other inline waiver for a rule or a reason, so the gate refuses each one,
  ...
  it off for its scope. It refuses the two attribute names in every `.csproj`, `.props`, `.targets`,
  `.wixproj` and `.slnx` file too, comments included. There an `AssemblyAttribute` item puts an
  attribute on the whole assembly, and a `Using` item aliases one. It refuses SA1404 there as well,
  except in a property's value, where a warning setting such as `WarningsAsErrors` names it, and in a
  comment. Roslyn reads a file as generated, and runs no analyzer over it, from a `<auto-generated>`
  or `<autogenerated>` comment or from its name, so the gate refuses the comment in a `.cs` file and
  a `.cs` file named `TemporaryGeneratedFile_*`, `*.designer.cs`, `*.generated.cs`, `*.g.cs` or
  `*.g.i.cs`. It refuses CSharpier's ignore comments in every file the `format` row checks.
- The directive match runs over the whole file, because C# ends a line at U+0085, U+2028 and U+2029
  ...
  compiler drops every format character from it, so `Generated­Code` binds to `[GeneratedCode]`. In a
  project file the match first reads the XML with character references decoded. It joins each
  element's text across any comment or CDATA section inside it, since MSBuild can join a value split
  that way. Then it decodes `%XX` escapes, since the SDK writes each item into C#.
...
Every row that starts dotnet, bun or a mise tool runs the same config checks first, so
...
- The gate starts mise with an environment it builds from nothing: `SYSTEMROOT`, `LOCALAPPDATA`,
  ...
  itself.
- The gate starts mise, gh, bun, dotnet and git from the absolute path `PATH` names for each,
...
- The prettier row runs `bun x --bun --no-install`, which starts `node_modules/.bin/prettier` ahead
  of anything else. With none in the checkout, `bun x` runs a parent directory's copy, one on `PATH`
  or one in its cache, and says nothing, so the row first checks that
  `node_modules/.bin/prettier.exe` is there, and that a link there resolves to an existing file, and
  fails naming the install to run when either does not hold. `--bun` runs it under the Bun the gate
  resolved, never whichever node `PATH` names, and lefthook's commit-msg hook passes it to
...
- `bunfig.toml` holds `[install]` with `minimumReleaseAge` alone. Bun reads the file on every
  ...
  prettier row refuse any other key or table before the gate starts bun. The file's lines, less
...
- The SARIF log records neither `NoWarn` nor `WarningsNotAsErrors`, so before each configuration
  ...
  - a `Compile` item that is not a `.cs` file the tree walk names, such as a file of another type,
    ...
    them;
  - a `Using` item, alias included, or an `AssemblyAttribute` item that names a refused attribute or
    SA1404, the justification rule. The text scan refuses one that spells the name out, and not one
    that builds it from properties;
  - an `Import` of a file that is not a `.csproj`, `.props`, `.targets`, `.wixproj` or `.slnx` file,
    or of one in a directory the tree walk skips (`.vs`, `.claude/worktrees` or the root
    `node_modules`), since the text scan reads none of the items it brings in;
...
- A local run that disagrees with continuous integration may have run another copy of a tool. The
  prettier row and the commit-msg hook start theirs with `bun x --bun --no-install`, which runs
  `node_modules/.bin/<tool>` in the checkout and never downloads. With no install there, the
  prettier row fails and names the install to run, and the hook runs the first copy `bun x` finds
  in a parent directory's `node_modules/.bin`, then on `PATH`, then in Bun's cache, and says nothing
