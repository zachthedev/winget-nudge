...
public static partial class Junction
{
    ...
    /// directory it made at <paramref name="link"/>.
    /// </remarks>
    /// <param name="link">The junction's path.</param>
    /// <param name="target">The directory the junction redirects to.</param>
    /// <exception cref="IOException">The directory could not be opened or made a junction.</exception>
    public static void Create(string link, string target)
    {
        ...
        if (handle.IsInvalid)
        {
            ...
            throw new IOException($"'{link}' could not be opened: {Marshal.GetPInvokeErrorMessage(error)}");
        }
